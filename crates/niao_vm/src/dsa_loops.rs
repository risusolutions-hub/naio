...
fn try_count_push(
    ...
    let OpCode::Load(i_slot) = code.get(head)? else {
        return None;
    };
    let OpCode::Load(limit_slot) = code.get(head + 1)? else {
        return None;
    };
    let OpCode::Lt = code.get(head + 2)? else {
        return None;
    };
    let OpCode::JumpIfFalse(exit) = code.get(head + 3)? else {
        return None;
    };
    let exit_ip = *exit as usize;

    let mut ip = head + 4;
    let OpCode::Load(ds_slot) = code.get(ip)? else {
        return None;
    };
    ip += 1;
    let (mul, add) = parse_linear(code, bc, &mut ip)?;
    let OpCode::Call { func, argc: 2 } = code.get(ip)? else {
        return None;
    };
    ...
    ip += 1;
    let OpCode::Pop = code.get(ip)? else {
        return None;
    };
    ip += 1;
    let OpCode::Load(i2) = code.get(ip)? else {
        return None;
    };
    if *i2 != *i_slot {
        return None;
    ...
    ip += 1;
    let OpCode::Add = code.get(ip)? else {
        return None;
    };
    ip += 1;
    let OpCode::Store(st) = code.get(ip)? else {
        return None;
    };
    if *st != *i_slot {
        return None;
    ...
    let OpCode::Jump(back) = code.get(jump_ip)? else {
        return None;
    };
...
fn try_drain_acc(
    ...
    let OpCode::Load(ds_slot) = code.get(head)? else {
        return None;
    };
    let OpCode::Call {
        func: empty_fn,
    ...
    let OpCode::Not = code.get(head + 2)? else {
        return None;
    };
    let OpCode::JumpIfFalse(exit) = code.get(head + 3)? else {
        return None;
    ...
    let exit_ip = *exit as usize;
    let OpCode::Load(acc_slot) = code.get(head + 4)? else {
        return None;
    };
    let OpCode::Load(ds2) = code.get(head + 5)? else {
        return None;
    };
    if *ds2 != *ds_slot {
        return None;
    ...
    let OpCode::Add = code.get(head + 7)? else {
        return None;
    };
    let OpCode::Store(acc2) = code.get(head + 8)? else {
        return None;
    };
    if *acc2 != *acc_slot {
        return None;
...
fn try_drain_count(
    ...
    let exit_ip = *exit as usize;
    let OpCode::Load(ds2) = code.get(head + 4)? else {
        return None;
    };
    if *ds2 != *ds_slot {
        return None;
    ...
    let OpCode::Pop = code.get(head + 6)? else {
        return None;
    };
    let OpCode::Load(acc_slot) = code.get(head + 7)? else {
        return None;
    };
    let OpCode::Const(one_idx) = code.get(head + 8)? else {
        return None;
    };
    if const_from_idx(_bc, *one_idx)? != 1 {
        return None;
    }
    let OpCode::Add = code.get(head + 9)? else {
        return None;
    };
    let OpCode::Store(acc2) = code.get(head + 10)? else {
        return None;
    };
    if *acc2 != *acc_slot {
        return None;
...
fn try_heap_drain_verify(
    ...
    let OpCode::Store(cur_slot) = code.get(head + 6)? else {
        return None;
    };
    let OpCode::Load(prev_slot) = code.get(head + 7)? else {
        return None;
    };
    let OpCode::Load(cur2) = code.get(head + 8)? else {
        return None;
    };
    if *cur2 != *cur_slot {
        return None;
    }
    let OpCode::Le = code.get(head + 9)? else {
        return None;
    };
    let OpCode::Const(_) = code.get(head + 10)? else {
        return None;
    };
    let OpCode::Call { argc: 2, .. } = code.get(head + 11)? else {
        return None;
    };
    let OpCode::Pop = code.get(head + 12)? else {
        return None;
    };
    let OpCode::Load(cur3) = code.get(head + 13)? else {
        return None;
    };
    if *cur3 != *cur_slot {
        return None;
    }
    let OpCode::Store(prev2) = code.get(head + 14)? else {
        return None;
    };
    if *prev2 != *prev_slot {
        return None;
...
fn try_set_lookup(
    ...
    let exit_ip = *exit as usize;
    let OpCode::Load(ds_slot) = code.get(head + 4)? else {
        return None;
    };
    let OpCode::Load(i2) = code.get(head + 5)? else {
        return None;
    };
    if *i2 != *i_slot {
        return None;
    }
    let OpCode::Call { func, argc: 2 } = code.get(head + 6)? else {
        return None;
    };
    if *fast_by_fidx.get(func)? != 20 {
        return None;
    }
    let OpCode::JumpIfFalse(skip) = code.get(head + 7)? else {
        return None;
    };
    let OpCode::Load(hits_slot) = code.get(head + 8)? else {
        return None;
    };
    let OpCode::Const(one) = code.get(head + 9)? else {
        return None;
    };
    if const_from_idx(bc, *one)? != 1 {
        return None;
    }
    let OpCode::Add = code.get(head + 10)? else {
        return None;
    };
    let OpCode::Store(h2) = code.get(head + 11)? else {
        return None;
    };
    if *h2 != *hits_slot {
        return None;
    ...
    let OpCode::Load(i3) = code.get(head + 12)? else {
        return None;
    };
    if *i3 != *i_slot {
        return None;
    ...
    let OpCode::Add = code.get(head + 14)? else {
        return None;
    };
    let OpCode::Store(i4) = code.get(head + 15)? else {
        return None;
    };
    if *i4 != *i_slot {
        return None;
...
fn try_map_lookup(
    ...
    let exit_ip = *exit as usize;
    let OpCode::Load(sum_slot) = code.get(head + 4)? else {
        return None;
    };
    let OpCode::Load(ds_slot) = code.get(head + 5)? else {
        return None;
    };
    let OpCode::Load(i2) = code.get(head + 6)? else {
        return None;
    };
    if *i2 != *i_slot {
        return None;
    }
    let OpCode::Call { func, argc: 2 } = code.get(head + 7)? else {
        return None;
    };
    if *fast_by_fidx.get(func)? != 22 {
        return None;
    }
    let OpCode::Add = code.get(head + 8)? else {
        return None;
    };
    let OpCode::Store(s2) = code.get(head + 9)? else {
        return None;
    };
    if *s2 != *sum_slot {
        return None;
    }
    let OpCode::Load(i3) = code.get(head + 10)? else {
        return None;
    };
    if *i3 != *i_slot {
        return None;
    ...
    let OpCode::Add = code.get(head + 12)? else {
        return None;
    };
    let OpCode::Store(i4) = code.get(head + 13)? else {
        return None;
    };
    if *i4 != *i_slot {
        return None;
...
fn try_map_build(
    ...
    let OpCode::Load(k1) = code.get(head + 5)? else {
        return None;
    };
    if *k1 != *i_slot {
        return None;
    }
    let OpCode::Load(v1) = code.get(head + 6)? else {
        return None;
    };
    if *v1 != *i_slot {
        return None;
    ...
    let OpCode::Mul = code.get(head + 8)? else {
        return None;
    };
    let OpCode::Call { func, argc: 3 } = code.get(head + 9)? else {
        return None;
    };
    if *fast_by_fidx.get(func)? != 21 {
        return None;
    }
    let OpCode::Pop = code.get(head + 10)? else {
        return None;
    };
    let OpCode::Load(i2) = code.get(head + 11)? else {
        return None;
    };
    if *i2 != *i_slot {
        return None;
    ...
    let OpCode::Add = code.get(head + 13)? else {
        return None;
    };
    let OpCode::Store(st) = code.get(head + 14)? else {
        return None;
    };
    if *st != *i_slot {
        return None;
...
fn try_graph_edges(
    ...
    let OpCode::Load(n_slot) = code.get(head + 1)? else {
        return None;
    };
    let OpCode::Const(one) = code.get(head + 2)? else {
        return None;
    };
    if const_from_idx(bc, *one)? != 1 {
        return None;
    }
    let OpCode::Sub = code.get(head + 3)? else {
        return None;
    };
    let OpCode::Lt = code.get(head + 4)? else {
        return None;
    };
    let OpCode::JumpIfFalse(exit) = code.get(head + 5)? else {
        return None;
    };
    let exit_ip = *exit as usize;
    let OpCode::Load(g_slot) = code.get(head + 6)? else {
        return None;
    };
    let OpCode::Load(i2) = code.get(head + 7)? else {
        return None;
    };
    if *i2 != *i_slot {
        return None;
    }
    let OpCode::Load(i3) = code.get(head + 8)? else {
        return None;
    };
    if *i3 != *i_slot {
        return None;
    ...
    let OpCode::Call { func, argc: 3 } = code.get(head + 11)? else {
        return None;
    };
    ...
    let OpCode::Load(i4) = code.get(head + 13)? else {
        return None;
    };
    if *i4 != *i_slot {
        return None;
    ...
    let OpCode::Add = code.get(head + 15)? else {
        return None;
    };
    let OpCode::Store(i5) = code.get(head + 16)? else {
        return None;
    };
    if *i5 != *i_slot {
        return None;
...
fn try_binary_search(
    ...
    let OpCode::Load(k_slot) = code.get(head + 1)? else {
        return None;
    };
    let OpCode::Lt = code.get(head + 2)? else {
        return None;
    ...
    let exit_ip = *exit as usize;
    let OpCode::Load(arr_slot) = code.get(head + 4)? else {
        return None;
    };
    let OpCode::Load(i2) = code.get(head + 5)? else {
        return None;
    ...
    let OpCode::Mul = code.get(head + 7)? else {
        return None;
    };
    let ip_after = head + 8;
    let OpCode::Call { func, argc: 2 } = code.get(ip_after)? else {
        return None;
    };
    if *fast_by_fidx.get(func)? != 25 {
        return None;
    }
    let OpCode::Const(m1) = code.get(ip_after + 1)? else {
        return None;
    };
    if const_from_idx(bc, *m1)? != -1 {
        return None;
    }
    let OpCode::Ge = code.get(ip_after + 2)? else {
        return None;
    };
    let OpCode::JumpIfFalse(skip) = code.get(ip_after + 3)? else {
        return None;
    };
    let OpCode::Load(_hits) = code.get(ip_after + 4)? else {
        return None;
    };
    let OpCode::Const(one) = code.get(ip_after + 5)? else {
        return None;
    };
    if const_from_idx(bc, *one)? != 1 {
        return None;
    }
    let OpCode::Add = code.get(ip_after + 6)? else {
        return None;
    };
    let OpCode::Store(h2) = code.get(ip_after + 7)? else {
        return None;
    };
    if *skip as usize != ip_after + 8 {
        return None;
    }
    let OpCode::Load(i3) = code.get(ip_after + 8)? else {
        return None;
    };
    if *i3 != *i_slot {
        return None;
    ...
    let OpCode::Add = code.get(ip_after + 10)? else {
        return None;
    };
    let OpCode::Store(i4) = code.get(ip_after + 11)? else {
        return None;
    };
    if *i4 != *i_slot {
        return None;
...
fn parse_linear(code: &[OpCode], bc: &[BytecodeConst], ip: &mut usize) -> Option<(i64, i64)> {
    let OpCode::Load(_) = code.get(*ip)? else {
        return None;
    };
    *ip += 1;
...
fn const_i64(code: &[OpCode], bc: &[BytecodeConst], ip: usize) -> Option<i64> {
    let OpCode::Const(idx) = code.get(ip)? else {
        return None;
    };
