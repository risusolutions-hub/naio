...
pub use registry::{store_callee, take_callee};
pub use sendable::{sendable_to_value_ref, value_to_sendable, SendableValue};

macro_rules! bind_fn {
    ($map:expr, $name:expr, $func:expr) => {
        $map.insert($name.to_string(), Value::NativeFunction($func).ref_cell());
    };
}
...
pub fn namespace() -> Value {
    ...
    map.insert(
        ...
        method_object(vec![
            ...
            (
                "recv_timeout",
                Rc::new(channel::parallel_channel_recv_timeout),
            ),
...
pub fn builtins() -> Vec<(&'static str, NativeFn)> {
    vec![
        // thread
        (
            "parallel_thread_spawn",
            Rc::new(thread::parallel_thread_spawn),
        ),
        (
            "parallel_thread_join",
            Rc::new(thread::parallel_thread_join),
        ),
        (
            "parallel_thread_detach",
            Rc::new(thread::parallel_thread_detach),
        ),
        (
            "parallel_thread_is_alive",
            Rc::new(thread::parallel_thread_is_alive),
        ),
        ("parallel_thread_id", Rc::new(thread::parallel_thread_id)),
        (
            "parallel_thread_yield",
            Rc::new(thread::parallel_thread_yield),
        ),
        (
            "parallel_thread_sleep",
            Rc::new(thread::parallel_thread_sleep),
        ),
        ...
        (
            "parallel_mutex_unlock",
            Rc::new(mutex::parallel_mutex_unlock),
        ),
        (
            "parallel_mutex_try_lock",
            Rc::new(mutex::parallel_mutex_try_lock),
        ),
        ...
        (
            ...
            Rc::new(channel::parallel_channel_new),
        ),
        (
            "parallel_channel_send",
            Rc::new(channel::parallel_channel_send),
        ),
        (
            "parallel_channel_recv",
            Rc::new(channel::parallel_channel_recv),
        ),
        (
            "parallel_channel_try_recv",
            Rc::new(channel::parallel_channel_try_recv),
        ),
        (
            "parallel_channel_close",
            Rc::new(channel::parallel_channel_close),
        ),
        (
            "parallel_channel_recv_timeout",
            Rc::new(channel::parallel_channel_recv_timeout),
        ),
        (
            "parallel_channel_is_closed",
            Rc::new(channel::parallel_channel_is_closed),
        ),
        // pool
        ...
        (
            ...
            Rc::new(pool::parallel_pool_shutdown),
        ),
