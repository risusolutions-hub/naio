...
pub fn from_ncl_handle(ncl_id: u64, span: Span) -> Result<u64, crate::RuntimeError> {
    with_handle(ncl_id, "nml_from_ncl", span, |h| match h {
        NclHandle::NDArray(arr) => {
            let data: Vec<f32> = arr
                ...
                .collect();
            let t =
                Tensor::from_cpu_data(&arr.shape, data, Device::Cpu).map_err(|e| e.to_string())?;
...
pub fn from_packed_float_array(
    args: &[ValueRef],
    span: Span,
) -> Result<Tensor, crate::RuntimeError> {
    ...
    Tensor::from_cpu_data(&shape, data, niao_tensor::global_device())
        .map_err(|e| crate::RuntimeError::at(span, codes::E1973_NML_SHAPE, e.to_string()))
}
