# Library spec: `jwt`  →  crate `niao_crypto`

| | |
|---|---|
| Category | Crypto |
| Replaces Rust crate(s) | `jsonwebtoken` (v9) |
| Target Niao crate | `crates/niao_crypto` |
| Difficulty | 2/5 — Medium |
| Status | ALREADY BUILT (tasks 01-12) — verify/extend only |
| Depends on Niao libs | sha2, hmac, base64 |

## Goal
Reimplement the functionality of `jsonwebtoken` as a **zero-external-dependency**, lightweight, high-performance native Niao/Rust module. Only `std` and existing `niao_*` crates allowed.

## Spec references
RFC 7519

## Implementation blueprint (make it FAST + LIGHT)
HS256/512 sign+verify, exp/nbf/iat, reject alg=none. RS256 out of scope.
...
## Risk / notes
RS256 still on jsonwebtoken if used
