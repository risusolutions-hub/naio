# Library spec: `base64`  →  crate `niao_codec`

| | |
|---|---|
| Category | Codec |
| Replaces Rust crate(s) | `base64,hex` (v0.22) |
...
## Goal
Reimplement the functionality of `base64,hex` as a **zero-external-dependency**, lightweight, high-performance native Niao/Rust module. Only `std` and existing `niao_*` crates allowed.

## Spec references
RFC 4648

## Implementation blueprint (make it FAST + LIGHT)
LUT-based encode/decode, std + url-safe alphabets, no-pad option, SIMD-friendly 4->3 byte batching.
...
## Performance target
>=700 MiB/s enc, >=780 dec (achieved)
