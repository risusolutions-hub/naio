# Library spec: `suppaftp`  →  crate `niao_net_clients`

| | |
|---|---|
| Category | Mail/FTP |
| Replaces Rust crate(s) | `suppaftp` (v6) |
...
## Goal
Reimplement the functionality of `suppaftp` as a **zero-external-dependency**, lightweight, high-performance native Niao/Rust module. Only `std` and existing `niao_*` crates allowed.

## Spec references
RFC 959

## Implementation blueprint (make it FAST + LIGHT)
FTP control+data channel, active/passive, LIST/RETR/STOR, TLS optional.
